...
def beam_translate(encoder, decoder, sentence, src_vocab, tgt_vocab, max_length=MAX_LENGTH, beam_size=5):
    """
    Beam-search decoding for the existing plain seq2seq model.
    This changes inference only: training remains the same.
    """
    encoder.eval()
    decoder.eval()
    ...
    with torch.no_grad():
        ...
        for _ in range(max_length):
            ...
            for score, token_ids, decoder_input, decoder_hidden, finished in beams:
                ...
                decoder_output, next_hidden = decoder(decoder_input, decoder_hidden)
...
def translate_sentences(encoder, decoder, pairs, src_vocab, tgt_vocab, max_num_sentences=None, max_length=MAX_LENGTH, beam_size=1):
    output_sentences = []
...
def main():
    ...
    ap.add_argument('--beam_size', default=1, type=int,
                help='beam size for inference/dev BLEU. Use 1 for greedy decoding, 5 for beam search')
    ap.add_argument('--dev_eval_size', default=500, type=int,
                help='number of dev sentences used for BLEU during training')
    ap.add_argument('--best_checkpoint_name', default='best_model.pt',
                help='filename for best dev-BLEU checkpoint inside checkpoint_dir')
    ...
    best_dev_bleu = state.get('best_dev_bleu', -1.0) if args.load_checkpoint is not None else -1.0
